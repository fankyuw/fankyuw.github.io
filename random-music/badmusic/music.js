...
musicFolders.forEach(folder =>{
    $.ajax({
        url:`/random-music/badmusic/${folder}/`,
        success:function(data){
            ...
            musicPaths.forEach(element => { 
                ...
                var newPath = newText.replaceAll(`/random-music/badmusic/${folder}/`,"")
                currentDiv.innerHTML = newPath.replaceAll("%20"," ")
                ...
                currentAudioSource.setAttribute("src",`/random-music/badmusic/${folder}/${newPath}`)
            }); 
            console.log('done')
        }
    })
